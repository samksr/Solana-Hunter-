...
function cooldownHost(hostOrObj, sec = 60) {
  const host = typeof hostOrObj === 'string' ? hostOrObj : (hostOrObj && hostOrObj.host);
...
const SENT_FILE = path.join(__dirname, 'sent_ids.json');

function loadSentIds() {
  try {
    if (fs.existsSync(SENT_FILE)) {
      const arr = JSON.parse(fs.readFileSync(SENT_FILE,'utf8') || '[]');
      arr.forEach(id => SENT_IDS.add(String(id)));
    }
  } catch (e) { /* ignore */ }
}
...
function persistSentIds() {
  try {
    const arr = Array.from(SENT_IDS).slice(-5000);
    fs.writeFileSync(SENT_FILE, JSON.stringify(arr, null, 2));
  } catch (e) { /* ignore */ }
}

loadSentIds();
setInterval(persistSentIds, 2 * 60 * 1000);

function markSent(id) {
  if (!id) return;
  SENT_IDS.add(String(id));
  if (SENT_IDS.size > 20000) {
    const keep = Array.from(SENT_IDS).slice(-5000);
    SENT_IDS.clear();
    keep.forEach(x => SENT_IDS.add(x));
  }
}
...
async function sendAdminDashboard(chatId) {
  try {
    ...
    await enqueue(chatId, txt, adminKeyboard());
  } catch (e) {
...
bot.on("callback_query", async q => {
  try {
    ...
    if (data === "DASH_VIEW_ALERTS") {
      ...
      await enqueue(cid, msg, { parse_mode: 'MarkdownV2' });
      return;
    }
    ...
    if (data === "DASH_RESTART") {
      ...
      await enqueue(cid, escapeMD("*Restarting bot as requested.*"));
      process.exit(0);
    }
