...
function loadHistory() {
  try {
    if (fs.existsSync(HISTORY_FILE)) {
      ...
      const now = Date.now();
      const CACHE_TTL = 172800000; // 48 hours - only keep recent tweets
      
      // Only load tweets from the last 48 hours to get fresh tweets
      data.forEach(item => {
...
function saveHistory() {
  try {
    const now = Date.now();
    const CACHE_TTL = 172800000; // 48 hours
    
    // Only save recent tweets (last 48 hours)
...
}, 60000); // Prune every 60 seconds instead of 120

loadHistory();

const agent = new https.Agent({ keepAlive: true, rejectUnauthorized: false });

// IP Rotation: User Agent pool for header randomization
const USER_AGENTS = [
...
function getRandomDelay() {
  return Math.random() * 2000 + 500; // 500-2500ms random delay
}
...
async function fetchWithRetry(urlPath, maxRetries = MAX_RETRIES) {
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    ...
    try {
      // IP Rotation: Randomize headers on each request to avoid blocking
